...
void FanManager::PostAttributeChangeCallback(EndpointId endpoint, ClusterId clusterId, AttributeId attributeId,
...
Status FanManager::HandleStep(FanControl::StepDirectionEnum aDirection, bool aWrap, bool aLowestOff)
{
    ChipLogProgress(NotSpecified, "FanManager::HandleStep aDirection %d, aWrap %d, aLowestOff %d",
...
void FanManager::HandleFanControlAttributeChange(AttributeId attributeId, uint8_t type, uint16_t size, uint8_t * value)
{
    switch (attributeId)
    {
...
void FanManager::HandleOnOff(AttributeId attributeId, uint8_t type, uint16_t size, uint8_t * value)
{
    ...
    if (on)
    {
        ...
        else if (percent.IsNull())
        {
            ...
            ChipLogError(NotSpecified,
                         "FanManager::HandleOnOff: PercentSetting is null when SpeedSetting is not. Setting PercentCurrent "
                         "from Speed");
        ...
        else if (speed.IsNull())
        {
            // This should never happen, but nonetheless, let's check and warn.
            ChipLogError(NotSpecified,
                         "FanManager::HandleOnOff: SpeedSetting is null when PercentSetting is not. Setting SpeedCurrent "
                         "from Percent");
...
void FanManager::PercentSettingWriteCallback(uint8_t aNewPercentSetting)
{
    ChipLogDetail(NotSpecified, "FanManager::PercentSettingWriteCallback: %d", static_cast<int>(aNewPercentSetting));
    if (mOnOffClusterOn)
    {
        ...
        {
            ChipLogError(NotSpecified,
                         "FanManager::PercentSettingWriteCallback: failed to set PercentCurrent attribute: %d",
                         to_underlying(status));
        }
    }
}

void FanManager::SpeedSettingWriteCallback(uint8_t aNewSpeedSetting)
{
    ChipLogDetail(NotSpecified, "FanManager::SpeedSettingWriteCallback: %d", static_cast<int>(aNewSpeedSetting));
    if (mOnOffClusterOn)
    {
        Status status = FanControl::Attributes::SpeedCurrent::Set(mEndpointId, aNewSpeedSetting);
        if (status != Status::Success)
        {
            ChipLogError(NotSpecified, "FanManager::SpeedSettingWriteCallback: failed to set SpeedCurrent attribute: %d",
                         to_underlying(status));
        }
    }
...
void FanManager::FanModeWriteCallback(FanControl::FanModeEnum aNewFanMode)
{
    ChipLogDetail(NotSpecified, "FanManager::FanModeWriteCallback: %d", (uint8_t) aNewFanMode);
    ...
    case FanControl::FanModeEnum::kAuto: {
        ChipLogProgress(NotSpecified, "FanManager::FanModeWriteCallback: Auto");
        break;
    }
    case FanControl::FanModeEnum::kUnknownEnumValue: {
        ChipLogProgress(NotSpecified, "FanManager::FanModeWriteCallback: Unknown");
        break;
    }
    }
}

void FanManager::SetSpeedSetting(DataModel::Nullable<uint8_t> aNewSpeedSetting)
{
    if (aNewSpeedSetting.IsNull())
    {
        ChipLogError(NotSpecified, "FanManager::SetSpeedSetting: invalid value");
        return;
    }
    ...
    if (status != Status::Success)
    {
        ChipLogError(NotSpecified, "FanManager::SetSpeedSetting: failed to set SpeedSetting attribute: %d",
                     to_underlying(status));
    }
}

DataModel::Nullable<uint8_t> FanManager::GetSpeedSetting()
{
    ...
    if (status != Status::Success)
    {
        ChipLogError(NotSpecified, "FanManager::GetSpeedSetting: failed to get SpeedSetting attribute: %d",
                     to_underlying(status));
    }
...
DataModel::Nullable<Percent> FanManager::GetPercentSetting()
{
    ...
    if (status != Status::Success)
    {
        ChipLogError(NotSpecified, "FanManager::GetPercentSetting: failed to get PercentSetting attribute: %d",
                     to_underlying(status));
    }
...
uint8_t FanManager::GetSpeedMax()
{
    uint8_t speedMax = 1;
    Status status    = FanControl::Attributes::SpeedMax::Get(mEndpointId, &speedMax);
    if (status != Status::Success)
    {
        ChipLogError(NotSpecified, "FanManager::GetPercentSetting: failed to get SpeedMax attribute: %d",
                     to_underlying(status));
    }
    return speedMax;
}
