...
from choice_conformance_support import evaluate_feature_choice_conformance
...
def _create_commands():
    ...
    xxx = ('    <command id="13" name="XXX" summary="summary">\n'
           '      <optionalConform />\n'
           '    </command>\n')
    xml.append(xxx)
    return '\n'.join(xml)
...
class TestConformanceSupport(MatterBaseTest):
    ...
    def _evaluate_problems(self, problems, expected_failures=list[str]):
        ...
        actual_failures = set([p.choice.choice for p in problems])
